/* ---------- Toys Products Script ---------- */
document.addEventListener("DOMContentLoaded", () => {
  const container = document.getElementById("toys-container"); 
  // make sure toys.html has <div id="toys-container"></div>

  async function loadToys() {
    try {
      let url = "/api/products/toys";
      ...
      toys.forEach(toy => {
        ...
        card.innerHTML = `
          <img src="${toy.image_url || 'default.jpg'}" alt="${toy.name}">
          <h3>${toy.name}</h3>
          <p class="price">₹${toy.price} / ${toy.unit}</p>
          <button class="add-to-cart" data-id="${toy.id}">Add to Cart</button>
        `;
      ...
      container.innerHTML = "<p>Failed to load toys. Please try again later.</p>";
    }
  }
