/* ---------- Dairy Products Script ---------- */
document.addEventListener("DOMContentLoaded", () => {
  const container = document.getElementById("dairy-container"); 
  // make sure dairy.html has <div id="dairy-container"></div>

  async function loadDairy() {
    try {
      let url = "/api/products/dairy";
      if (currentSubcat !== "all") {
        url += `?subcategory=${encodeURIComponent(currentSubcat)}`;
      }
      const res = await fetch(url);

      const dairyItems = await res.json();
      ...
      console.error("Error loading dairy products:", err);
      container.innerHTML = "<p>Failed to load dairy products. Please try again later.</p>";
    }
  }
  ...
  subcatBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // update subcat + reload dairy items
