/* ---------- Stationary Products Script ---------- */
document.addEventListener("DOMContentLoaded", () => {
  const container = document.getElementById("stationary-container"); 
  // make sure stationary.html has <div id="stationary-container"></div>

  async function loadStationary() {
    try {
      let url = "/api/products/stationary";
      if (currentSubcat !== "all") {
        url += `?subcategory=${encodeURIComponent(currentSubcat)}`;
      }
      const res = await fetch(url);

      const stationaryItems = await res.json();
      ...
      console.error("Error loading stationary products:", err);
      container.innerHTML = "<p>Failed to load stationary products. Please try again later.</p>";
    }
  }
  ...
  subcatBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // update subcat + reload stationary items
