/* ---------- Masala & Dry Products Script ---------- */
document.addEventListener("DOMContentLoaded", () => {
  const container = document.getElementById("masaladry-container"); 
  // make sure masaladry.html has <div id="masaladry-container"></div>

  async function loadMasalaDry() {
    try {
      let url = "/api/products/masaladry";
      if (currentSubcat !== "all") {
        url += `?subcategory=${encodeURIComponent(currentSubcat)}`;
      }
      const res = await fetch(url);

      const masalaItems = await res.json();
      ...
      console.error("Error loading masala & dry products:", err);
      container.innerHTML = "<p>Failed to load masala & dry products. Please try again later.</p>";
    }
  }
  ...
  subcatBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // update subcat + reload masala & dry items
