...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // make sure HTML has <div id="personal-container"></div>

  async function loadPersonal() {
    try {
      let url = "/api/products/personal";
      if (currentSubcat !== "all") {
        url += `?subcategory=${encodeURIComponent(currentSubcat)}`;
      }
      const res = await fetch(url);
      const personals = await res.json();
      ...
      console.error("Error loading personal products:", err);
      container.innerHTML = "<p>Failed to load personal products. Please try again later.</p>";
    }
  }
