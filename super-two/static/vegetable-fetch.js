/* ---------- Vegetable Products Script ---------- */
document.addEventListener("DOMContentLoaded", () => {
  const container = document.getElementById("vegetable-container"); 
  // make sure vegetable.html has <div id="vegetable-container"></div>

  async function loadVegetables() {
    try {
      let url = "/api/products/vegetables";
      if (currentSubcat !== "all") {
        url += `?subcategory=${encodeURIComponent(currentSubcat)}`;
      }
      const res = await fetch(url);

      const vegetableItems = await res.json();
      ...
      console.error("Error loading vegetable products:", err);
      container.innerHTML = "<p>Failed to load vegetable products. Please try again later.</p>";
    }
  }
  ...
  subcatBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // update subcat + reload vegetable items
